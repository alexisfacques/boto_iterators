...
# From local modules:
from .type_hints import IteratorChain, Result
...
def iterate():
    ...
    def lambda_handler(iterator_chain_fn: IteratorChain) -> Callable[[Dict[str, Any], Optional[Any]],
        ...
        def wrapper(event: Dict[str, Any], context: Optional[Any] = None) -> Tuple[Result, ...]:
            """
            ...
            if 'Records' not in event or not isinstance(event['Records'], list):
                ...
                generator = (((json.loads(record['body']) if 'body' in record else record), prev)
            ...
            for bolt in (iterator_chain_fn() if args_count == 0
                         ...
                         else iterator_chain_fn(event, context) if args_count == 2
                         else iterator_chain_fn(event, context, resolve)):
